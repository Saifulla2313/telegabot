...
async def create_trial_subscription(
    ...
    # Автоматически назначаем суточный тариф, если tariff_id не указан
    if tariff_id is None:
        try:
            from app.database.crud.tariff import get_first_daily_tariff
            daily_tariff = await get_first_daily_tariff(db)
            if daily_tariff:
                tariff_id = daily_tariff.id
                logger.info(f"🔄 Автоматически назначен суточный тариф {daily_tariff.name} (ID: {tariff_id}) для пользователя {user_id}")
        except Exception as e:
            logger.debug(f"Не удалось получить суточный тариф: {e}")
