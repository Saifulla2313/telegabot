...
async def create_user(
    ...
    for attempt in range(1, attempts + 1):
        ...
        from app.config import settings
        welcome_bonus = settings.WELCOME_BONUS_KOPEKS if settings.WELCOME_BONUS_KOPEKS > 0 else 0
        ...
        try:
            ...
            if welcome_bonus > 0:
                logger.info(
                    f"🎁 Пользователю {telegram_id} начислен приветственный бонус: {welcome_bonus / 100}₽"
                )
                
                # Создаем транзакцию для приветственного бонуса
                try:
                    from app.database.models import Transaction, TransactionType
                    
                    bonus_transaction = Transaction(
                        user_id=user.id,
                        amount_kopeks=welcome_bonus,
                        type=TransactionType.BONUS.value,
                        description=settings.WELCOME_BONUS_DESCRIPTION,
                        is_completed=True,
                    )
                    db.add(bonus_transaction)
                    await db.commit()
                except Exception as tx_error:
                    logger.warning(f"Не удалось создать транзакцию приветственного бонуса: {tx_error}")
            ...
            try:
                ...
                await event_emitter.emit(
                    "user.created",
                    {
                        ...
                        "welcome_bonus_kopeks": welcome_bonus,
                    },
                    db=db,
                )
