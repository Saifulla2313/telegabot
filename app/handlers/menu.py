...
def _get_subscription_status(user: User, texts, is_daily_tariff: bool = False) -> str:
    ...
    if actual_status == "active":
        # Для суточных тарифов показываем только статус без предупреждения об истечении
        if is_daily_tariff:
            # Проверяем, приостановлена ли подписка
            is_paused = getattr(subscription, "is_daily_paused", False)
            if is_paused:
                return texts.t("SUB_STATUS_DAILY_PAUSED", "⏸️ Приостановлена")
            return texts.t("SUB_STATUS_DAILY_ACTIVE", "💎 Активна (суточная)")
...
async def get_main_menu_text(user, texts, db: AsyncSession):
    ...
    # Проверяем суточный тариф независимо от режима тарифов
    if subscription and subscription.tariff_id:
        try:
            ...
            if tariff:
                ...
                logger.info(f"📋 Тариф загружен: {tariff.name}, is_daily={is_daily_tariff}")
                # Формируем краткий блок информации о тарифе для главного меню (только в режиме тарифов)
                if settings.is_tariffs_mode():
                    tariff_info_block = f"\n📦 Тариф: {tariff.name}"
            else:
                logger.warning(f"📋 Тариф с id={subscription.tariff_id} не найден")
        except Exception as e:
            logger.error(f"Не удалось загрузить тариф для главного меню: {e}")
    else:
        logger.info(f"📋 Подписка: {subscription}, tariff_id: {getattr(subscription, 'tariff_id', None) if subscription else None}")
