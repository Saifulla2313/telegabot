...
async def complete_registration_from_callback(
    ...
    if existing_user and existing_user.status == UserStatus.DELETED.value:
        ...
        # Автоматически создаем триальную подписку для восстановленного пользователя
        if not user.subscription:
            try:
                ...
                logger.info(f"🎁 Автоматически создана триальная подписка для восстановленного пользователя {user.telegram_id}")
            except Exception as trial_error:
                logger.error(f"Ошибка при создании автоматической триальной подписки: {trial_error}")
        ...
        # Автоматически создаем триальную подписку для нового пользователя
        try:
            from app.database.crud.subscription import create_trial_subscription
            subscription = await create_trial_subscription(db, user.id)
            ...
            await db.refresh(user, ['subscription'])
            logger.info(f"🎁 Автоматически создана триальная подписка для пользователя {user.telegram_id}")
        except Exception as trial_error:
            logger.error(f"Ошибка при создании автоматической триальной подписки: {trial_error}")
        
        # Уведомление о приветственном бонусе
        if settings.WELCOME_BONUS_KOPEKS > 0:
            try:
                bonus_amount = texts.format_price(settings.WELCOME_BONUS_KOPEKS)
                welcome_bonus_msg = texts.t(
                    "WELCOME_BONUS_RECEIVED",
                    "🎁 <b>Добро пожаловать!</b>\n\nВам начислен приветственный бонус: <b>{amount}</b>!\n\nИспользуйте его для покупки подписки."
                ).format(amount=bonus_amount)
                await callback.message.answer(welcome_bonus_msg)
            except Exception as bonus_msg_error:
                logger.warning(f"Не удалось отправить уведомление о приветственном бонусе: {bonus_msg_error}")
    else:
    ...
    if existing_user and existing_user.status == UserStatus.DELETED.value:
        ...
        if settings.WELCOME_BONUS_KOPEKS > 0:
            try:
                ...
                await message.answer(welcome_bonus_msg)
            except Exception as bonus_msg_error:
                logger.warning(f"Не удалось отправить уведомление о приветственном бонусе: {bonus_msg_error}")
    else:
